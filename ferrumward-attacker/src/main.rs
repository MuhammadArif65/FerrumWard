...
mod linux_impl {
    ...
    pub fn run() {
        ...
        if args.len() < 3 || args[1] != "--attack" {
            eprintln!("Usage: ferrumward-attacker --attack <ptrace|scan|inject|freeze>");
            process::exit(1);
        }

        let mode = args[2].as_str();

        println!("[*] Scanning for ferrumward-mock-game...");
        let mut sys = System::new_all();
        sys.refresh_all();

        let target_pid = sys.processes().iter().find_map(|(&pid, p)| {
            if p.name().to_string_lossy().contains("ferrumward-mock") {
        ...
        let pid = match target_pid {
            Some(p) => p,
            None => {
                eprintln!("[-] ferrumward-mock-game is not running!");
                process::exit(1);
            }
        };
...
mod stub {
    pub fn run() {
        println!("ferrumward-attacker is only supported on Linux.");
    }
}
