...
unsafe impl ExtensionLibrary for RustShieldGodot {}

#[derive(GodotClass)]
#[class(base=Node)]
pub struct RustShieldGuard {
    base: Base<Node>,
}

#[godot_api]
impl INode for RustShieldGuard {
...
#[godot_api]
impl RustShieldGuard {
    #[signal]
    fn tamper_detected();

    /// Initializes the RustShield protection engine.
    ...
    fn initialize(
        ...
        // Since the closure needs to be Send + Sync, we can't capture `Gd<RustShieldGuard>` directly.
        ...
        let config = ProtectionConfig {
            ...
            anti_debug,
            anti_vm,
            on_failure: Some(Box::new(move |_err: RustShieldError| {
                ...
                if let Ok(mut obj) = Gd::<RustShieldGuard>::try_from_instance_id(instance_id) {
                    obj.call_deferred(
                        "emit_signal",
