...
fn test_cli_keygen_and_license() {
    let nanos = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("rustshield_cli_test_keygen_{}", nanos));
    ...
    // 1. Keygen
    let status = Command::new(env!("CARGO_BIN_EXE_rustshield"))
        .arg("keygen")
        .arg("--private-key")
        .arg(&priv_key)
        .arg("--public-key")
...
fn test_cli_manifest() {
    let nanos = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("rustshield_cli_test_manifest_{}", nanos));
    fs::create_dir_all(&dir).unwrap();
