...
/// Initializes the RustShield protection engine from C/C++.
...
pub extern "C" fn rustshield_init(config: *const CProtectionConfig) -> i32 {
    let result = catch_unwind(AssertUnwindSafe(|| {
        ...
        let rust_config = ProtectionConfig {
            ...
            on_failure: failure_cb.map(|cb| {
                let wrapped_cb: Box<dyn Fn(RustShieldError) + Send + Sync> =
                    Box::new(move |_err| {
                        cb();
                    });
                wrapped_cb
...
#[no_mangle]
pub extern "C" fn rustshield_run_checkpoint() -> i32 {
    let result = catch_unwind(AssertUnwindSafe(|| match rustshield_checkpoint!() {
        Ok(_) => 1,
        Err(_) => 0,
    }));
...
#[no_mangle]
pub extern "C" fn rustshield_disable_checks() {
    let _ = catch_unwind(AssertUnwindSafe(|| {
        if let Some(state) = rustshield_core::protection::integrity::get_active_state() {
