...
pub fn hash_file(path: &Path) -> Result<String> {
    let mut file = File::open(path).map_err(RustShieldError::Io)?;
    let mut hasher = Sha256::new();
    let mut buffer = [0u8; CHUNK_SIZE];

    loop {
        let count = file.read(&mut buffer).map_err(RustShieldError::Io)?;
        if count == 0 {
            break;
        }
...
pub fn verify_manifest(game_dir: &Path, manifest_path: &Path) -> Result<IntegrityReport> {
    let manifest_file = File::open(manifest_path).map_err(RustShieldError::Io)?;
    let expected_hashes: HashMap<String, String> =
        serde_json::from_reader(manifest_file).map_err(|_| RustShieldError::ManifestCorrupted)?;
