...
pub fn init_self_check() -> Result<()> {
    let exe_path = env::current_exe().map_err(|_| RustShieldError::TamperDetected)?;

    let mut file = File::open(&exe_path).map_err(|_| RustShieldError::TamperDetected)?;
...
pub fn verify_self_check() -> Result<()> {
    let original = ORIGINAL_HASH.get().ok_or(RustShieldError::TamperDetected)?;

    let exe_path = env::current_exe().map_err(|_| RustShieldError::TamperDetected)?;

    let mut file = match File::open(&exe_path) {
        Ok(f) => f,
        Err(_) => return Err(RustShieldError::TamperDetected), // File is missing/locked?
    };
