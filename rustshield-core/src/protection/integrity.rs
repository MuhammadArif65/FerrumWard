...
/// Configuration for the RustShield protection system.
pub struct ProtectionConfig {
    ...
    pub on_failure: Option<Box<dyn Fn(RustShieldError) + Send + Sync>>,
}

/// Global state to support `rustshield_checkpoint!` macros.
...
/// Entry point to initialize RustShield protection.
...
pub fn protect(config: ProtectionConfig) -> Result<()> {
    ...
    }) as Box<dyn Fn(RustShieldError) + Send + Sync>);
...
fn trigger_failure(config: &ProtectionConfig, _real_error: RustShieldError) -> Result<()> {
...
fn verify_hwid_silent() -> Result<()> {
    ...
    match get_hardware_id() {
        ...
        _ => Err(RustShieldError::TamperDetected),
    }
}
