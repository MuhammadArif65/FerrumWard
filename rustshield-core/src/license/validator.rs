...
///
/// # Errors
/// Returns an appropriate `RustShieldError` such as `InvalidSignature`, `HardwareMismatch`,
...
pub(crate) fn validate_license(
    ...
    if parts.len() != 2 {
        return Err(RustShieldError::MalformedLicense);
    }
    ...
    let payload_bytes = URL_SAFE_NO_PAD
        .decode(payload_b64)
        .map_err(|_| RustShieldError::MalformedLicense)?;
    ...
    // 3. Verify signature
    let signature =
        Signature::from_slice(&sig_bytes).map_err(|_| RustShieldError::MalformedLicense)?;

    public_key
        .verify(&payload_bytes, &signature)
        .map_err(|_| RustShieldError::InvalidSignature)?;

    // 4. Parse the payload into LicenseData
    let license_data: LicenseData =
        serde_json::from_slice(&payload_bytes).map_err(|_| RustShieldError::MalformedLicense)?;

    // 5. Verify Game ID
    if license_data.game_id != expected_game_id {
        return Err(RustShieldError::GameIdMismatch);
    }
    ...
    let expected_json = String::from_utf8(
        URL_SAFE_NO_PAD
            ...
            .map_err(|_| RustShieldError::HardwareMismatch)?,
    )
    .map_err(|_| RustShieldError::HardwareMismatch)?;

    let expected_profile: HwidProfile =
        serde_json::from_str(&expected_json).map_err(|_| RustShieldError::HardwareMismatch)?;
    ...
    if score < 4 {
        return Err(RustShieldError::HardwareMismatch);
    }
    ...
    if let Some(expiry) = license_data.expires_at {
        ...
        if current_time >= expiry {
            return Err(RustShieldError::LicenseExpired);
        }
    }
